...
public class SupportController {
    private final AuthService authService; private final FaqService faqService; private final TicketService ticketService; private final TicketNotificationService notificationService; private final TicketAssignmentService assignmentService; private final KnowledgePermissionService knowledgePermissionService;
    public SupportController(AuthService authService,FaqService faqService,TicketService ticketService,TicketNotificationService notificationService,TicketAssignmentService assignmentService,KnowledgePermissionService knowledgePermissionService){this.authService=authService;this.faqService=faqService;this.ticketService=ticketService;this.notificationService=notificationService;this.assignmentService=assignmentService;this.knowledgePermissionService=knowledgePermissionService;}
    ...
    @GetMapping("/processor/knowledge-permission") public Map<String,Object> knowledgePermissionStatus(@RequestHeader(value="Authorization",required=false)String a){return knowledgePermissionService.status(requireProcessor(a));}
    @PostMapping("/processor/knowledge-permission/requests") public Map<String,Object> requestKnowledgePermission(@RequestHeader(value="Authorization",required=false)String a,@RequestBody(required=false)Map<String,Object>b){try{Long id=knowledgePermissionService.request(requireProcessor(a),b==null?"":String.valueOf(b.getOrDefault("reason","")));return Map.of("success",true,"requestId",id);}catch(IllegalArgumentException e){throw new ResponseStatusException(HttpStatus.BAD_REQUEST,e.getMessage());}}
    @PostMapping("/processor/faqs") public Map<String,Object> createProcessorFaq(@RequestHeader(value="Authorization",required=false)String a,@RequestBody Map<String,Object>b){AuthService.Session p=requireProcessor(a);try{knowledgePermissionService.requireGranted(p);}catch(SecurityException e){throw new ResponseStatusException(HttpStatus.FORBIDDEN,e.getMessage());}Long id=faqService.create(p,b);return Map.of("success",true,"id",id);}

    @GetMapping("/admin/knowledge-permission/requests") public List<Map<String,Object>> knowledgePermissionRequests(@RequestHeader(value="Authorization",required=false)String a){requireAdmin(a);return knowledgePermissionService.listForAdmin();}
    @PostMapping("/admin/knowledge-permission/requests/{id}/review") public Map<String,Object> reviewKnowledgePermission(@RequestHeader(value="Authorization",required=false)String a,@PathVariable Long id,@RequestBody Map<String,Object>b){AuthService.Session admin=requireAdmin(a);boolean approve=Boolean.TRUE.equals(b.get("approve"))||"true".equalsIgnoreCase(String.valueOf(b.get("approve")));try{knowledgePermissionService.review(admin,id,approve);return Map.of("success",true,"approved",approve);}catch(IllegalArgumentException e){throw new ResponseStatusException(HttpStatus.BAD_REQUEST,e.getMessage());}}
    ...
    private AuthService.Session requireProcessor(String a){AuthService.Session s=requireUser(a);if(!"processor".equals(s.role()))throw new ResponseStatusException(HttpStatus.FORBIDDEN,"仅处理人员可以使用该功能");return s;}
