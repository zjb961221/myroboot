...
public class DatabaseMigration {
    ...
    public void migrate() {
        ...
        addColumnIfMissing("support_ticket", "assigned_to", "BIGINT NULL");
        addColumnIfMissing("support_ticket", "assigned_by", "BIGINT NULL");
        addColumnIfMissing("support_ticket", "assigned_time", "DATETIME NULL");
        ...
        createIndexIfMissing("support_ticket", "idx_ticket_assigned", "CREATE INDEX idx_ticket_assigned ON support_ticket(assigned_to,is_deleted,status,id)");
        ...
        executeOptional("ticket_notification table", "CREATE TABLE IF NOT EXISTS ticket_notification (id BIGINT PRIMARY KEY AUTO_INCREMENT,ticket_id BIGINT NOT NULL,notification_type VARCHAR(50) NOT NULL,status VARCHAR(20) NOT NULL DEFAULT 'pending',recipient_email VARCHAR(200),attempt_count INT NOT NULL DEFAULT 0,last_error VARCHAR(1000),sent_time DATETIME,create_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,update_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY uk_ticket_notification (ticket_id,notification_type),INDEX idx_ticket_notification_status(status,update_time))");
