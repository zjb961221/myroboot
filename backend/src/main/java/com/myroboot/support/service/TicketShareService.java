...
public class TicketShareService {
    ...
    public Map<String, Object> create(AuthService.Session session, Long ticketId, Integer requestedHours) {
        ...
        jdbcTemplate.update("INSERT INTO ticket_share(ticket_id,token_hash,created_by,expires_time) VALUES (?,?,?,?)",
        ...
        result.put("id", id); result.put("ticketId", ticketId); result.put("token", token); result.put("expiresTime", expires);
        return result;
    }
    ...
    public List<Map<String, Object>> list(AuthService.Session session, Long ticketId) {
        ...
        if ("admin".equals(session.role())) {
            return jdbcTemplate.queryForList("SELECT id,ticket_id,created_by,expires_time,revoked,revoked_time,access_count,last_access_time,create_time FROM ticket_share WHERE ticket_id=? ORDER BY id DESC", ticketId);
        }
        return jdbcTemplate.queryForList("SELECT id,ticket_id,created_by,expires_time,revoked,revoked_time,access_count,last_access_time,create_time FROM ticket_share WHERE ticket_id=? AND created_by=? ORDER BY id DESC", ticketId, session.userId());
    }

    @Transactional
    public boolean revoke(AuthService.Session session, Long ticketId, Long shareId) {
        requireTicketAccess(session, ticketId);
        List<Map<String, Object>> rows = jdbcTemplate.queryForList("SELECT id,created_by,revoked FROM ticket_share WHERE id=? AND ticket_id=? FOR UPDATE", shareId, ticketId);
    ...
    public ShareAccess requireValid(String rawToken) {
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT s.id,s.ticket_id,s.expires_time FROM ticket_share s JOIN support_ticket t ON t.id=s.ticket_id WHERE s.token_hash=? AND s.revoked=0 AND s.expires_time>NOW() AND t.is_deleted=0 LIMIT 1",
                sha256(rawToken.trim()));
        if (rows.isEmpty()) throw new SecurityException("分享链接无效或已失效");
        Map<String, Object> row = rows.get(0);
        return new ShareAccess(((Number) row.get("id")).longValue(), ((Number) row.get("ticket_id")).longValue(), toLocalDateTime(row.get("expires_time")));
    }

    public Map<String, Object> sharedTicket(ShareAccess access) {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT t.id,t.customer_name,t.mine_name,t.category,t.description,t.status,t.screenshot_url,t.create_time," +
        ...
        result.put("customer_name", source.get("customer_name"));
        result.put("mine_name", source.get("mine_name"));
        result.put("submitter_name", fallback(source.get("submitter_name"), "未知提交人"));
        result.put("processor_name", fallback(source.get("processor_name"), "暂未分配"));
        ...
        List<Map<String, Object>> attachments = jdbcTemplate.queryForList("SELECT id,original_name,content_type,file_size FROM ticket_attachment WHERE ticket_id=? ORDER BY id", access.ticketId());
        for (Map<String, Object> attachment : attachments) attachment.put("file_url", "/api/public/ticket-share/attachments/" + attachment.get("id"));
        result.put("attachments", attachments);
        return result;
    }

    public Map<String, Object> attachment(ShareAccess access, Long attachmentId) {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList("SELECT id,file_url,original_name,content_type,file_size FROM ticket_attachment WHERE id=? AND ticket_id=?", attachmentId, access.ticketId());
        if (rows.isEmpty()) throw new SecurityException("附件不存在或无权访问");
        return rows.get(0);
    }

    public String screenshotUrl(ShareAccess access) {
        List<String> rows = jdbcTemplate.query("SELECT screenshot_url FROM support_ticket WHERE id=? AND is_deleted=0", (rs, rowNum) -> rs.getString(1), access.ticketId());
        if (rows.isEmpty() || rows.get(0) == null || rows.get(0).isBlank()) throw new SecurityException("截图不存在");
        return rows.get(0);
    }

    private Map<String, Object> requireTicketAccess(AuthService.Session session, Long ticketId) {
        List<Map<String, Object>> rows = jdbcTemplate.queryForList("SELECT id,user_id,is_deleted FROM support_ticket WHERE id=?", ticketId);
        if (rows.isEmpty() || asBoolean(rows.get(0).get("is_deleted"))) throw new IllegalArgumentException("工单不存在");
        Map<String, Object> row = rows.get(0);
        if (!"admin".equals(session.role())) {
            Object owner = row.get("user_id");
            if (!(owner instanceof Number) || ((Number) owner).longValue() != session.userId()) throw new SecurityException("无权分享该工单");
        }
        return row;
    }
