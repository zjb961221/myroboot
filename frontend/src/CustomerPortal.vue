...
function isVideo(file) { return String(file.content_type || file.contentType || '').startsWith('video/') || /\.(mp4|mov|avi|mkv|webm)$/i.test(file.original_name || file.name || '') }
  ...
  <main class="page">
    ...
    <template v-if="customerTab==='knowledge'">
      ...
      <section class="content"><div class="panel list-panel"><div class="panel-title"><h2>匹配问题</h2><span>{{ loading ? '搜索中...' : `${faqs.length} 条` }}</span></div><button v-for="item in faqs" :key="item.id" class="faq-item" @click="openFaq(item)"><span class="category">{{ item.category }}</span><strong>{{ item.question }}</strong><span class="arrow">›</span></button><div v-if="!loading&&!faqs.length" class="empty">暂时没有匹配到问题，可以换个关键词，或提交工单。</div></div>
        <div class="panel detail-panel"><template v-if="selected"><span class="category">{{ selected.category }}</span><h2>{{ selected.question }}</h2><div class="answer rich-content" v-html="selected.answer"></div><div v-if="selected.images?.length" class="knowledge-images"><a v-for="image in selected.images" :key="image" :href="image" target="_blank"><img :src="image" /></a></div><div v-if="selected.attachments?.length" class="knowledge-files"><h3>相关资料</h3><template v-for="file in selected.attachments" :key="file.id"><video v-if="isVideo(file)" :src="file.file_url" controls preload="metadata"></video><a :href="file.file_url" target="_blank"><span>{{ file.original_name }}</span><small>{{ fileSize(file.file_size) }}</small></a></template></div><div class="resolved-box"><strong>按上面的方案操作后，问题解决了吗？</strong><div class="actions"><button class="success" @click="selected=null">已经解决</button><button class="secondary" @click="unresolved">没有解决，提交工单</button></div></div></template><template v-else><div class="placeholder"><h2>先选择一个问题</h2><p>这里会显示完整解决方案、步骤、图片和相关资料。</p><button class="secondary" @click="showTicket=true">确实找不到，提交问题</button></div></template></div></section>
