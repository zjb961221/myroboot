...
function validateTicket() {
  if (!form.value.customerName.trim()) { showToast('请填写客户名称'); return false }
  if (!form.value.mineName.trim()) { showToast('请填写矿井名称'); return false }
  if (!form.value.category.trim()) { showToast('请填写问题类型'); return false }
  if (!form.value.screenshotUrl) { showToast('请上传故障截图'); return false }
  if (!form.value.description.trim()) { showToast('请填写问题描述'); return false }
  if (form.value.description.trim().length < 5) { showToast('问题描述太短，请至少写清楚现象或报错信息'); return false }
  if (!form.value.attachments.length) { showToast('请至少上传 1 个工单附件'); return false }
  return true
}
...
<template>
  <main class="page">
    <section class="hero"><div><span class="badge">MYROBOOT SUPPORT</span><h1>客户自助技术支持</h1><p>先查问题库，标准方案无法解决时再提交工单。</p></div><div class="header-actions customer-actions"><span>{{ profile.displayName || username }}<small>{{ profile.mineName || profile.companyName }}</small></span><a v-if="isAdmin" class="primary link-button" href="/admin">进入管理后台</a><button class="secondary" @click="logout">退出</button></div></section>
    ...
    <template v-if="customerTab==='knowledge'">
      ...
      <section v-if="showTicket" id="ticket-form" class="panel ticket-panel"><div v-if="submittedTicketId" class="submitted"><h2>问题已提交</h2><p>工单编号：#{{ submittedTicketId }}</p><button class="primary" @click="customerTab='tickets'">查看我的工单</button></div><template v-else><h2>第二步：提交完整故障信息</h2><div class="form-grid"><label>客户名称 <em class="required">*</em><input v-model="form.customerName" /></label><label>矿井名称 <em class="required">*</em><input v-model="form.mineName" /></label><label>问题类型 <em class="required">*</em><input v-model="form.category" placeholder="APP / 视频 / 数据 / 网络" /></label><label>故障截图 <em class="required">*</em><input type="file" accept="image/*" @change="chooseTicketImage" /><small>{{ ticketUploading ? '上传中...' : '必填，请上传最关键的一张报错截图' }}</small></label></div><div v-if="form.screenshotUrl" class="ticket-preview"><img :src="form.screenshotUrl" /></div><label class="full">问题描述 <em class="required">*</em><textarea v-model="form.description" rows="7" placeholder="请写清发生时间、报错信息、影响范围、已经尝试过什么"></textarea></label><div class="attachment-box"><div class="attachment-head"><div><strong>提交附件 <em class="required">*</em></strong><small>必填，至少上传 1 个附件。支持图片、日志、Excel、Word、PPT、PDF、压缩包和视频；普通文件≤30MB，视频≤200MB，最多 10 个。</small></div><label class="file-button">{{ attachmentUploading ? '上传中...' : '选择附件' }}<input type="file" multiple @change="chooseTicketAttachments" /></label></div><div v-if="form.attachments.length" class="ticket-files"><div v-for="(file,index) in form.attachments" :key="file.url" class="ticket-file"><div><strong>{{ file.name }}</strong><small>{{ fileSize(file.size) }}</small></div><button class="danger small-btn" @click="removeAttachment(index)">移除</button></div></div></div><div v-if="similarFaqs.length" id="similar-box" class="similar-box"><h3>提交前发现这些相似问题</h3><p>建议先确认标准方案，避免重复工单。</p><button v-for="item in similarFaqs" :key="item.id" @click="reviewSimilar(item)"><span class="category">{{item.category}}</span><strong>{{item.question}}</strong></button><div class="actions"><button class="primary" @click="submitTicket(true)">仍然提交工单</button><button class="secondary" @click="similarFaqs=[]">返回修改</button></div></div><button v-else class="primary submit-ticket" :disabled="similarChecking||ticketUploading||attachmentUploading" @click="submitTicket(false)">{{similarChecking?'正在检查相似问题...':'提交技术支持工单'}}</button></template></section>
    </template>

    <template v-else>
      <section class="panel ticket-history"><div class="panel-title"><div><h2>我的工单</h2><p>查看处理进度和最终解决回执。</p></div><button class="secondary" @click="loadMyTickets">刷新</button></div><div v-if="!myTickets.length" class="empty">暂时没有提交过工单。</div><article v-for="ticket in myTickets" :key="ticket.id" class="my-ticket"><div class="ticket-main"><div><span class="status-pill" :class="ticket.status">{{statusText(ticket.status)}}</span><strong>#{{ticket.id}} · {{ticket.category}}</strong><p>{{ticket.description}}</p><small>提交时间 {{formatTime(ticket.create_time)}}</small></div><div class="ticket-actions"><button v-if="ticket.status==='pending'||ticket.status==='processing'" class="danger small-btn" @click="cancelTicket(ticket)">撤销工单</button><a class="secondary link-button" :href="`/ticket-detail?id=${ticket.id}`">查看处理进度</a></div></div><div v-if="ticket.status==='resolved'" class="receipt"><strong>解决回执</strong><p>{{ticket.resolution||'已处理完成'}}</p><small>解决时间 {{formatTime(ticket.resolved_time)}}</small></div><div v-else-if="ticket.status==='cancelled'" class="cancelled-receipt"><strong>工单已撤销</strong><p>{{ticket.cancel_reason||'客户主动撤销工单'}}</p><small>撤销时间 {{formatTime(ticket.cancelled_time)}}</small></div><div v-else class="waiting-receipt"><span>处理中</span><p>技术人员正在处理，后续进展会显示在工单详情时间线中。</p></div></article></section>
    </template>
  </main>
</template>
