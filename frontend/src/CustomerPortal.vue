...
const form = ref({
  customerName: profile.value.companyName || profile.value.mineName || '',
...
async function loadFaq() {
  ...
  try {
    ...
    faqs.value = await request(q ? `${apiBase}/faq/search?q=${encodeURIComponent(q)}` : `${apiBase}/faq`)
  } catch (e) {
    showToast(e.message)
  } finally {
    loading.value = false
...
async function loadSuggestions() {
  ...
  try {
    suggestions.value = await request(`${apiBase}/faq/suggest?q=${encodeURIComponent(q)}`)
  } catch {
    suggestions.value = []
  } finally {
...
async function chooseSuggestion(item) {
  keyword.value = item.question
  await loadFaq()
  selected.value = faqs.value.find(x => x.id === item.id) || faqs.value[0] || null
}
...
async function chooseTicketImage(event) {
  ...
  try {
    const data = await request(`${apiBase}/upload/image`, { method: 'POST', body: fd })
    form.value.screenshotUrl = data.url
    showToast('故障截图已上传', 'success')
  } catch (e) {
    showToast(e.message)
  } finally {
...
async function submitTicket(force = false) {
  ...
  if (!force) {
    ...
    try {
      const query = `${form.value.category} ${form.value.description}`.trim()
      similarFaqs.value = await request(`${apiBase}/ticket/similar?q=${encodeURIComponent(query)}`)
      if (similarFaqs.value.length) {
        setTimeout(() => document.getElementById('similar-box')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50)
        return
      }
    } catch (e) {
      showToast(e.message)
  ...
  try {
    ...
    similarFaqs.value = []
    await loadMyTickets()
    showToast(`工单 #${data.ticketId} 已提交`, 'success')
  } catch (e) {
    showToast(e.message)
  }
}

function reviewSimilar(item) {
  selected.value = item
  showTicket.value = false
  similarFaqs.value = []
  setTimeout(() => document.querySelector('.detail-panel')?.scrollIntoView({ behavior: 'smooth' }), 50)
}

async function loadMyTickets() {
  try { myTickets.value = await request(`${apiBase}/tickets/mine`) } catch (e) { showToast(e.message) }
}
...
watch(customerTab, value => {
  if (value === 'tickets') loadMyTickets()
})
  ...
  <main class="page">
    <section class="hero">
      <div>
        ...
        <h1>客户自助技术支持</h1>
        <p>先查问题库，标准方案无法解决时再提交工单。</p>
      </div>
      <div class="header-actions customer-actions">
        <span>{{ profile.displayName || username }}<small>{{ profile.mineName || profile.companyName }}</small></span>
        <button class="secondary" @click="logout">退出</button>
      </div>
    </section>

    <section class="flow-panel">
      <div class="flow-step"><span>1</span><div><strong>搜索问题库</strong><small>实时匹配分类、标题、内容和关键词</small></div></div>
    ...
    <div class="customer-tabs">
      <button :class="{active:customerTab==='knowledge'}" @click="customerTab='knowledge'">问题库 / 提交问题</button>
      <button :class="{active:customerTab==='tickets'}" @click="customerTab='tickets'">我的工单 <span v-if="myTickets.length">{{ myTickets.length }}</span></button>
    </div>

    <template v-if="customerTab==='knowledge'">
      <section class="search-section panel">
        <div><h2>第一步：先从问题库查找</h2><p>输入报错、功能名称或现象，系统会实时给出候选问题。</p></div>
        <div class="search-autocomplete">
          <div class="search-box light">
            <input v-model="keyword" @keyup.enter="loadFaq" placeholder="例如：视频黑屏、APP 登录不上、数据未上报" autocomplete="off" />
            <button @click="loadFaq">搜索</button>
          </div>
          <div v-if="keyword.trim() && (suggestions.length || suggesting)" class="suggestion-menu">
            <div v-if="suggesting" class="suggestion-loading">正在匹配...</div>
            <button v-for="item in suggestions" :key="item.id" @click="chooseSuggestion(item)">
              <span class="category">{{ item.category }}</span><strong>{{ item.question }}</strong>
            </button>
          </div>
        </div>
      </section>
      ...
      <section class="content">
        <div class="panel list-panel">
          <div class="panel-title"><h2>匹配问题</h2><span>{{ loading ? '搜索中...' : `${faqs.length} 条` }}</span></div>
          ...
          <div v-if="!loading&&!faqs.length" class="empty">暂时没有匹配到问题，可以换个关键词，或提交工单。</div>
        </div>

        <div class="panel detail-panel">
          <template v-if="selected">
            <span class="category">{{ selected.category }}</span><h2>{{ selected.question }}</h2>
            <div class="answer rich-content" v-html="selected.answer"></div>
            ...
            <div class="resolved-box"><strong>按上面的方案操作后，问题解决了吗？</strong><div class="actions"><button class="success" @click="selected=null">已经解决</button><button class="secondary" @click="unresolved">没有解决，提交工单</button></div></div>
          </template>
          <template v-else><div class="placeholder"><h2>先选择一个问题</h2><p>这里会显示完整解决方案、步骤和图片。</p><button class="secondary" @click="showTicket=true">确实找不到，提交问题</button></div></template>
        </div>
      </section>

      <section v-if="showTicket" id="ticket-form" class="panel ticket-panel">
        <div v-if="submittedTicketId" class="submitted"><h2>问题已提交</h2><p>工单编号：#{{ submittedTicketId }}</p><button class="primary" @click="customerTab='tickets'">查看我的工单</button></div>
        <template v-else>
          <h2>第二步：提交完整故障信息</h2>
          <div class="form-grid">
          ...
          <div v-if="similarFaqs.length" id="similar-box" class="similar-box">
            ...
            <button v-for="item in similarFaqs" :key="item.id" class="similar-item" @click="reviewSimilar(item)"><span class="category">{{ item.category }}</span><strong>{{ item.question }}</strong><span>查看方案 →</span></button>
            <div class="similar-actions"><button class="secondary" @click="similarFaqs=[]">返回修改描述</button><button class="primary" @click="submitTicket(true)">这些都不是，仍然提交</button></div>
          </div>
        </template>
      </section>
    </template>

    <section v-else class="ticket-history">
      <div class="panel-title"><h2>我的工单</h2><span>{{ myTickets.length }} 条</span></div>
      <div v-if="!myTickets.length" class="panel empty">暂时没有提交过工单。</div>
      <article v-for="ticket in myTickets" :key="ticket.id" class="panel ticket-card">
