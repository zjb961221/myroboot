...
async function load(){error.value='';try{const [list,metadata]=await Promise.all([json(listUrl()),json(`${apiBase}/tickets/${id}/metadata`)]);const base=list.find(x=>String(x.id)===String(id))||null;if(!base)throw new Error('工单不存在或无权查看');ticket.value={...base,...metadata};history.value=await json(historyUrl());reason.value=ticket.value.resolution_reason||'';result.value=ticket.value.resolution_result||'';await Promise.all([loadShares(),loadProcessors()])}catch(e){error.value=e.message}}
...
<main class="page">
  ...
  <section v-if="ticket" class="panel">
    ...
    <h3 class="problem-title">问题描述</h3>
