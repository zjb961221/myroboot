...
import ProcessorKnowledge from './ProcessorKnowledge.vue'
...
<template>
  <main class="page">
    <header>
      <div><span>MYROBOOT PROCESSOR</span><h1>技术支持工作台</h1><p>处理分配给你的工单，并把可复用的解决经验沉淀到问题库。</p></div>
      <div class="user"><div><b>{{profile.displayName||username}}</b><small>处理人员</small></div><button @click="logout">退出</button></div>
    </header>

    <nav class="tabs">
      <button :class="{active:tab==='tickets'}" @click="tab='tickets'">我的工单</button>
      <button :class="{active:tab==='knowledge'}" @click="tab='knowledge'">问题库录入</button>
    </nav>
