...
const form = ref({ role:'', username:'', email:'', code:'', password:'', confirmPassword:'', displayName:'', companyName:'', mineName:'', phone:'' })
...
async function register(){
  ...
  if(!form.value.role) return error.value='请选择注册角色：客户或处理人员'
  ...
  if(form.value.role==='customer' && !form.value.mineName.trim()) return error.value='请填写矿井'
...
<template>
  <main class="register-page"><section class="register-card">
    <div class="top"><div><span class="badge">MYROBOOT SUPPORT</span><h1>创建账号</h1><p>请选择账号角色。客户可查询问题库和提交工单；处理人员登录后只处理分配给自己的工单。</p></div><a href="/">返回登录</a></div>
    <div class="role-section"><strong>注册角色 <em>*</em></strong><div class="role-options"><label :class="{selected:form.role==='customer'}"><input v-model="form.role" type="radio" value="customer" /><span><b>客户</b><small>查询问题库、提交工单、选择处理人员、查看处理回执</small></span></label><label :class="{selected:form.role==='processor'}"><input v-model="form.role" type="radio" value="processor" /><span><b>处理人员</b><small>接收工单邮件，只查看和处理分配给自己的工单</small></span></label></div></div>
    <div class="grid">
      ...
      <label>单位 <em>*</em><input v-model="form.companyName" required :placeholder="form.role==='processor'?'技术支持单位/部门':'公司/单位名称'" /></label>
      <label v-if="form.role!=='processor'">矿井 <em>*</em><input v-model="form.mineName" required placeholder="矿井名称" /></label><label v-else>账号用途<input value="技术支持工单处理" disabled /></label>
...
<style scoped>
.register-page{min-height:100vh;padding:42px 20px;background:#f4f7fb}.register-card{max-width:900px;margin:auto;background:#fff;border:1px solid #e1e8f0;border-radius:22px;padding:34px;box-shadow:0 20px 60px rgba(20,40,70,.07)}.top{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;margin-bottom:24px}.badge{font-size:12px;letter-spacing:1.4px;color:#245eea}.top h1{font-size:32px;margin:10px 0}.top p{color:#718096;line-height:1.7;max-width:650px}.top a{color:#245eea;text-decoration:none;white-space:nowrap}.role-section{margin-bottom:22px}.role-section>strong{display:block;color:#34455d;margin-bottom:10px}.role-section em,.grid label em{color:#d92d20;font-style:normal}.role-options{display:grid;grid-template-columns:1fr 1fr;gap:12px}.role-options label{display:flex;gap:12px;align-items:flex-start;border:1px solid #d7dfeb;border-radius:12px;padding:14px;cursor:pointer}.role-options label.selected{border-color:#245eea;background:#f4f7ff}.role-options input{margin-top:4px}.role-options span{display:grid;gap:4px}.role-options small{color:#718096;line-height:1.5}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.grid label{display:grid;gap:8px;font-weight:600;color:#34455d}.grid input{width:100%;border:1px solid #d7dfeb;border-radius:10px;padding:12px;outline:none}.grid input:focus{border-color:#245eea}.grid input:disabled{background:#f6f8fb;color:#7a8798}.code-row{display:flex;gap:8px}.code-row input{flex:1}.code-row button,.submit{border:0;border-radius:10px;background:#245eea;color:#fff;padding:0 16px;white-space:nowrap}.code-row button:disabled,.submit:disabled{opacity:.55}.submit{margin-top:22px;padding:13px 22px}.error,.message{margin-top:16px;padding:12px 14px;border-radius:10px}.error{background:#fff4f2;border:1px solid #fecdca;color:#b42318;display:grid;gap:3px}.error span{font-size:14px}.message{background:#edf9f2;color:#18794e}@media(max-width:720px){.grid,.role-options{grid-template-columns:1fr}.grid>span{display:none}.top{display:block}.top a{display:inline-block;margin-top:10px}.register-card{padding:24px}.code-row{align-items:stretch}.code-row button{padding:0 12px}}
</style>
